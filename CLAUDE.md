...
- The default ESP8266 watchdogs are relied on unchanged, but they are blind to loops that call `delay()`/`yield()`. Any potentially unbounded wait must therefore carry its own timeout — `startWifi()` gives up after 60 s and calls `ESP.restart()` (safe: `setup()` reconnects and the first `loop()` pass refetches the calendar).
